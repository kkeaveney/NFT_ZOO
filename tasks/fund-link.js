...
task("fund-link", "Funds a contract with LINK")
    ...
    .setAction(async (taskArgs) => {
        ...
        let linkTokenAddress = networkConfig[networkId]['linkToken'] || taskArgs.linkToken
        const LinkToken = await ethers.getContractFactory("LinkToken")

        // Fund with 1 LINK token
        const amount = web.utils.toHex(1e18)

        // Get signer information
        const accounts = await ethers.getSigners()
        const signer = accounts[0]

        // Create connection to LINK token contract and iniate the transfer
